Shader "Custom/GrassTrampleRenderTexture_ShadowReceiving"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // // Convert world position to UV relative to the trample map
                // float2 trampleUV = input.positionWS.xz / float2(2000, 2000);
                // trampleUV = saturate(trampleUV); // clamp to 0–1

                // // Sample the trample map
                // float4 t = SAMPLE_TEXTURE2D(_TrampleMap, sampler_TrampleMap, trampleUV);

                // half4 col;

                // // If the trample map's red channel is nonzero, color red; otherwise green
                // if (t.r > 0.01)
                //     col = half4(1, 0, 0, 1); // trampled area
                // else
                //     col = half4(0, 1, 0, 1); // untrampled area

                // return col;

                // // --- Sample normal albedo ---
                // half4 albedo = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, input.uv);
                // albedo *= _Color * input.color;
                // clip(albedo.a - _Cutoff);

                // // --- Force red at a specific world position ---
                // float2 targetPos = float2(100.0, 150.0); // X,Z in world space
                // float radius = 5.0; // How far from the target point to affect
                // float2 currentPos = float2(input.positionWS.x, input.positionWS.z);

                // float dist = distance(currentPos, targetPos);
                // if (dist <= radius)
                // {
                //     // Blend with red based on distance
                //     float t = 1.0 - (dist / radius);
                //     albedo.rgb = lerp(albedo.rgb, float3(1,0,0), t);
                // }
                ...
                // If you see RED tint, trample map is working!
                // Remove these 2 lines after testing
                if (trampleDebug.r > 0.01)
                    albedo.rgb = lerp(albedo.rgb, float3(1, 0, 0), trampleDebug.r * 0.5);
